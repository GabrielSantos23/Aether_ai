      ...
      return (
        <div className="my-4 last:mb-0 group relative overflow-x-auto max-w-[87vw] md:max-w-full">
          <div className="flex items-center justify-between bg-black/5 dark:bg-white/5 px-4 py-2 rounded-t-lg border-b border-black/10 dark:border-white/10">
            ...
            <button
              onClick={handleCopy}
              className="opacity-0 group-hover:opacity-100 text-xs text-black/50 dark:text-white/50 hover:text-purple-500 dark:hover:text-purple-300 transition-all duration-200 px-2 py-1 rounded"
            >
              {copied ? "Copied!" : "Copy"}
            </button>
          </div>
...
const components: Partial<Components> = {
  ...
  code: ({ children, node, className, ...props }) => {
    ...
    return (
      <code
        className={cn(
          "rounded bg-muted px-[0.3rem] py-[0.2rem] font-mono text-sm overflow-x-auto",
          className
        )}
        {...props}
      >
