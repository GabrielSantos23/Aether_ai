            ...
            <>
              ...
              {call.result && call.result.success && call.result.imageUrl && (
                <div className="relative group w-fit">
                  {/*
                    Some images are returned as lightweight HTML wrappers containing
                    a base64-encoded <img>. When this happens we detect the
                    `isHtmlWrapper` flag (set by the backend) and render the
                    wrapper inside an <iframe>. Otherwise we render the image
                    directly with an <img> tag.
                  */}
                  {call.result.isHtmlWrapper ? (
                    <iframe
                      src={call.result.imageUrl}
                      title={call.args.prompt}
                      className="w-auto h-auto max-h-96 max-w-full rounded-lg border border-black/10 dark:border-white/10"
                    />
                  ) : (
                    <img
                      src={call.result.imageUrl}
          ...
          {call.toolName === "startDeepResearch" && (
            <div>
              <div className="flex items-center gap-2 text-sm text-black/60 dark:text-white/60 mb-2">
                ...
                <span>
                  {call.result
                    ? "Initiated deep research on:"
                    : "Starting deep research on:"}{" "}
                  <em>"{call.args.topic || call.args.query || "topic"}"</em>
                </span>
              </div>
