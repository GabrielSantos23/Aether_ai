...
export function DataSettings({ unmigratedLocalChats }: DataSettingsProps) {
  ...
  const handleMigrate = async () => {
    ...
    const migrationToast = toast.loading(
      `Importing ${unmigratedLocalChats.length} chats...`
    );
    ...
    try {
      ...
      toast.error("An error occurred during chat import.", {
        id: migrationToast,
      });
    } finally {
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      toast.error("An error occurred while deleting your data.", {
        id: deleteToast,
      });
    } finally {
      setIsDeleting(false);
    ...
    <div className="space-y-6">
      {unmigratedLocalChats && unmigratedLocalChats.length > 0 && (
        <div className="p-4 rounded-xl bg-muted/20 backdrop-blur-sm border border-border/50">
          ...
          <p className="text-sm text-blue-600/80 dark:text-blue-300/80 mb-3">
            You have {unmigratedLocalChats.length} conversation
            {unmigratedLocalChats.length > 1 ? "s" : ""} from a previous
            session. Import them to your account.
          </p>
          <Button
      ...
      <div>
        ...
        <div className="space-y-3 rounded-xl bg-muted/20 backdrop-blur-sm border border-border/50 p-4">
          ...
          >
            {isExporting ? (
              <Spinner className="w-4 h-4 mr-3 animate-spin" />
            ) : (
      ...
      <div>
        ...
        <div className="p-4 rounded-xl bg-muted/20 backdrop-blur-sm border border-border/50">
          ...
          <p className="text-sm text-red-600/80 dark:text-red-300/80 mb-3">
            Deleting your conversations is a permanent action and cannot be
            undone.
          </p>
          <Button
            variant="destructive"
