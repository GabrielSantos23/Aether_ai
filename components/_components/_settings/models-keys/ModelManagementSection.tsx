...
export const ModelManagementSection = ({
  ...
  // Group models - FIX: Use the correct key based on groupBy value
  const groupedModels = filteredModels.reduce(
    (acc, model) => {
      // Use the correct grouping key based on the groupBy state
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-between">
          <input
            ...
            className="w-full bg-muted/30 border border-border rounded-lg px-3 py-2.5 pr-10 text-sm text-foreground placeholder:text-muted-foreground focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500/50 outline-none transition-all duration-200 backdrop-blur-sm"
          />
                        ...
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {["web", "vision", "code", "imagegen"].map(
                                (feature) =>
                                  model.features.includes(feature as any) && (
                                    <span
                                      key={feature}
                                      className="text-xs text-purple-500/60 dark:text-purple-300/60 bg-purple-100/50 dark:bg-purple-900/30 px-1.5 py-0.5 rounded-full capitalize"
                                    >
                                      {feature}
                                    </span>
                                  )
