...
export const LLMProviderKeysSection = ({
  title,
  provider,
  keys,
  onSave,
  ...
  return (
    <SectionWrapper
      title={title}
      description={`Manage your API keys for ${title.replace(" Keys", "")}`}
      addKeyButton={
        <Button
          variant="outline"
          size="sm"
      ...
      <div className="space-y-1.5 mt-2">
        {providerKeys.length > 0
          ? providerKeys.map((apiKey) => (
              <div
                key={apiKey._id}
                className="p-2 rounded-md bg-muted/30 border border-border/40 flex items-center justify-between"
              >
