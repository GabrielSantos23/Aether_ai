...
export function ModelsKeysSettings({
  ...
  const [optimisticDisabledModels, setOptimisticDisabledModels] = useState<
    string[]
  >([]);
  ...
  // Only update from server disabled models when they change and aren't empty
  useEffect(() => {
    if (serverDisabledModels && serverDisabledModels.length > 0) {
      setOptimisticDisabledModels(serverDisabledModels);
    }
  }, [JSON.stringify(serverDisabledModels)]);

  // Use server disabled models if optimistic ones are empty
  const disabledModels =
    optimisticDisabledModels.length > 0
      ? optimisticDisabledModels
      : serverDisabledModels;

  // Only update local settings when user settings change
  useEffect(() => {
    if (userSettings) {
      setLocalSettings((prevSettings: Partial<UserSettings>) => {
        // Only update if different to avoid unnecessary re-renders
        if (JSON.stringify(prevSettings) !== JSON.stringify(userSettings)) {
          return userSettings;
        }
        return prevSettings;
      });
  ...
  const debouncedUpdateSettings = useCallback(
    debounce(
      (settings: Partial<UserSettings>) => updateSettings(settings),
      500
    ),
    [updateSettings]
  );
        ...
        <TabsContent value="keys" className="space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Add your own API keys to access premium models. When available,
              your keys will be used automatically instead of our system keys.
            </p>
          </div>
