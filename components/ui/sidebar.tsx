...
import { mergeButtonRefs } from "@/components/ui/merge-button-refs";
  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            "h-svh w-(--sidebar-width) bg-transparent transition-[width] ease-snappy",
        ...
        <div
          className={cn(
            "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] ease-snappy md:flex",
            side === "left"
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col overflow-auto group-data-[collapsible=icon]:overflow-hidden small-scrollbar scroll-shadow relative",
        className
      )}
      {...props}
    />
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-color-heading outline-hidden ring-sidebar-ring transition-[margin,opacity] ease-snappy focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
