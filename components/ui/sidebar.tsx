...
function setLocalStorageItem(key: string, value: boolean): void {
...
function SidebarProvider({
  ...
  const [_openLeft, _setOpenLeft] = useState(() =>
    getLocalStorageItem(
  ...
  const [_openRight, _setOpenRight] = useState(() =>
    getLocalStorageItem(`${SIDEBAR_STORAGE_KEY}_right`, defaultOpenRight)
  );
  ...
  const setOpenLeft = useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      setLocalStorageItem(`${SIDEBAR_STORAGE_KEY}_left`, openState);
    },
  ...
  const setOpenRight = useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Save to localStorage
      setLocalStorageItem(`${SIDEBAR_STORAGE_KEY}_right`, openState);
    },
...
function SidebarTrigger({
  ...
  return (
    ...
    >
      <PanelLeftIcon
        className={cn({
          "rotate-180 w-5 h-5": targetSide === "right",
        })}
      />
