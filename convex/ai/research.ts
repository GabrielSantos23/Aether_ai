...
import { checkUserCredits } from "../users";
...
const performResearch = async (
  ...
  // Retrieve user's OpenRouter API key if available
  const userOpenRouterKey = await opts.ctx.runQuery(
  ...
  const { text, usage } = await generateText({
    model: getModel(
      "openrouter",
      "moonshotai/kimi-k2",
      userOpenRouterKey as string | undefined
    ),
